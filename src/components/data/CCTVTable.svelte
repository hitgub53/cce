<script lang="ts">
  ...
  import { CCTVItem } from "./../../models/CCTVItem";
  import {
    cameras,
    codecs,
    cctvItems,
  ...
  function deleteEntry(item: CCTVItem) {
    let items: CCTVItem[] = [];
    ...
    <tbody>
      {#each $cctvItems as item}
        <tr>
            ...
            <div class="flex">
              ...
              >
                {#each $cameras as camera}
                  <option value={camera}>
                    {camera.pixelsHorizontal} px * {camera.pixelsVertical} px * {camera.pixelDepth}
                    Bit
                  </option>
                {/each}
              </select>
          ...
          <td>
            <strong class="input is-static">{item.terabytes} TB</strong>
          </td>
          <td style="text-align: end">
            <button class="button is-black" on:click={() => deleteEntry(item)}>
