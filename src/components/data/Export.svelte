<script lang="ts">
  ...
  function exportData() {
    ...
    $cctvItems.forEach((element) => {
      csv += `"${element.camrea.pixelsVertical}*${element.camrea.pixelsHorizontal}*${element.camrea.pixelDepth}",`;
      csv += `"${element.codec.codecName}","${element.fps}","${element.recHoursPerDay}","${element.recDays}"`;
      csv += `,"${element.quantity}","${dotToComma(element.terabytes)}"\n`;
    });
